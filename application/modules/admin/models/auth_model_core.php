...
class Auth_model_core extends CI_Model 
{
	...
	function check_login($user_name,$password,$return_as='num_rows')
	{
		...
		$query = $this->db->get_where('dbc_users',array('user_name'=>$user_name,'password'=>$password));

		if($query->num_rows()<=0)
		{
			
			$mail_query = $this->db->get_where('dbc_users',array('user_email'=>$user_name,'password'=>$password));
	...
	function set_login_cookie($user_name)
	{
		...
		$this->db->update('dbc_users',$data,array('user_name'=>$user_name));
	}
	
	function check_cookie_val($user,$key)
	{
		$query = $this->db->get_where('dbc_users',array('user_name'=>$user,'remember_me_key'=>$key));
		if($query->num_rows()>0)
		{
	...
	{
		$query = $this->db->get_where('dbc_users',array('user_email'=>$email));
		return $query->num_rows();
	}

    function is_username_exists($username)
    {
        $query = $this->db->get_where('dbc_users',array('user_name'=>$username));
        return $query->num_rows();
    }
	...
	function set_recovery_key($user_email)
	{
		...
		$this->db->update('dbc_users',$data,array('user_email'=>$user_email));
		
		$query = $this->db->get_where('dbc_users',array('user_email'=>$user_email));
		$row = $query->row();
	...
	function verify_recovery($user_name,$recovery_key)
	{
		...
		{
			$query = $this->db->get_where('dbc_users',array('user_name'=>$user_name,'recovery_key'=>$recovery_key));
			return $query->num_rows();
		}
	}
    ...
    function insert_user($data){
        $this->db->insert('dbc_users', $data);
    }

	function confirm_email($email,$code)
	{
		$query = $this->db->get_where('dbc_users',array('user_email'=>$email,'confirmation_key'=>$code));
		if($query->num_rows()>0)
		{
