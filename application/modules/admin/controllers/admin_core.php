...
class Admin_core extends CI_Controller {
	...
	{
		$value['profile']	= $this->admin_model->get_user_profile_by_id($this->session->userdata('user_id'));
	...
	public function updateprofile(){
            ...
            else
            {
                ...
                if($this->input->post('password') != ''){
                    ...
                    $userdata['passwd'] = $this->encrypt->sha1($this->input->post('password'));
                }

                add_user_meta($id,'coins', $this->input->post('coins'));
	...
	public function deleteuser($page='0',$id='',$confirmation='')
	{
		...
		if($confirmation=='')
		{
			$data['content'] = load_admin_view('confirmation_view',array('num'=>$id,'url'=>site_url('admin/deleteuser/'.$page)),TRUE);
			load_admin_view('template/template_view',$data);
		}
		else
		{
