...
class Payment_core extends MX_Controller {
	...
	public function feature_payment_2checkout_callback()

	{

		    $this->load->model('user/payment_model');

		    $order = $this->payment_model->get_post_payment_data_by_unique_id(trim($_REQUEST['merchant_order_id']));
...

		    {
		    	...
		    	$package = $this->package_model->get_package_by_id($order->package_id);
		    		...
		    		$response = serialize($_REQUEST);
		    			...
		    			$data['response_log']		= 'Success package unique id:'.trim($_REQUEST['merchant_order_id']);



		    			$this->payment_model->update_post_payment_data_by_unique_id($data,trim($_REQUEST['merchant_order_id']));
