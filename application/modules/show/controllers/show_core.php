...
class Show_core extends CI_controller {
	...
	{	
		...
		{
			$row = $value['post']->row();
            $value['similar_posts'] = $this->post_model->get_similar_posts_by_category($row->category, $row->parent_category, $row->id);
	...
	public function sendemailtoagent($agent_id='0')
	{
		...
		else
		{
			...
			$data['msg'] 		  = $this->input->post('msg').'<br/>'.lang_key('phone').': '.$this->input->post('phone');
			
			$data['msg']		 .= "<br /><br /> Это сообщение было отправлено со страницы объявления:<br /><a href=\"".$detail_link."\" target=\"_blank\">".site_url('ads/'.$unique_id.'/'.$title)."</a>";
			...
			$msg = '<table width="600" cellpadding="0" cellspacing="0" style="font-family:Arial,Helvetica,sans-serif; font-size: 14px; color: #666;" align="center">
	<tbody>
		<tr>
		...
		<tr>
			<td width="600" align="center" style="background: #FFF; line-height: 18px; padding: 10px;  border-bottom: 1px solid #DDD; border-left: 1px solid #DDD; border-right: 1px solid #DDD;">
				...
				<p style="margin-bottom: 10px;">
						Здравствуйте, вы получили новое сообщение со страницы вашего <strong><a style="text-decoration: none; color: #3498db;" href="'.$detail_link.'">объявления</a></strong>.<br/><br/><strong>Текст сообщения:</strong> '.$this->input->post('msg').'<br/><p style="text-decoration: none; color: #FFF; font-weight: bold; padding: 12px 20px; background: #3498db; border-radius: 3px; -webkit-border-radius: 3px;">'.lang_key('contact-email').': '.$this->input->post('phone').', <a style="text-decoration: none; color: #fff;">'.$this->input->post('sender_email').'</a></p>'; 
			$msg .= '</p><p style="margin-bottom: 10px;">Это и другие сообщения вы также можете прочитать в своем аккаунте в разделе <a style="text-decoration: none; color: #3498db;" href="http://isra-go.com/index.php/admin/classified/emailtracker">личные сообщения</a>.</p>
					<p style="text-align: center; font-size: 11px; padding: 5px 0 0 0;">
						С уважением, команда <a style="text-decoration: none; color: #3498db;" href="http://isra-go.com/" target="_blank">ISRA-GO</a>! ☺
					</p>
					<p style="text-align: center; font-size: 10px;">Это письмо было отправлено автоматически, на него отвечать не нужно!</p>
			</td>
		</tr>
	</tbody>
</table>';
			...
			$this->load_contact_agent_view($unique_id,$msg);
    ...
    public function get_city_dropdown_by_parent_ajax($parent='')
    {
    	...
    	else
    	{
	    	...
	    	foreach ($query->result() as $row) 
	    	{
	    		echo '<option city_id="'.$row->id.'" value="'.$row->name.'">'.$row->name.'</option>';
	    	}    		
    	}
