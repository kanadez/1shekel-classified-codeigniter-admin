                    ...
                    ?>
                        <div class="row">
                                    ...
                                    <div class="col-md-9 col-sm-9 col-xs-12">
                                        ...
                                        <?php }?>
                                        <a href="<?php echo site_url('profile/'.$user->id.'/'.get_user_fullname_by_id($user->id));?>"><span class="label label-red"><?php echo lang_key('posts');?>: <?php echo get_user_properties_count($user->id);?></span></a>
                                        <?php if(get_user_meta($user->id, 'about_me')!=''){?>
                                            <!-- Para -->
                                            <p><?php echo get_user_meta($user->id, 'about_me'); ?></p>
                                        <?php }?>
                                        <p class="contact-types">
                                            <?php if(get_settings('classified_settings','hide_users_profile_phone','no')!='yes'){?>
                                            <strong><?php echo lang_key('phone'); ?>:</strong> <?php echo get_user_meta($user->id, 'phone'); ?> 
                                            <?php } ?>
                                            <?php if(get_settings('classified_settings','hide_users_profile_email','no')!='yes'){?>
                                            <strong><?php echo lang_key('email');?>:</strong> <a href="mailto:<?php echo $user->user_email; ?>"><?php echo $user->user_email; ?></a>
                                            <?php }?>
                                        </p>
                                        <!-- Social -->
                                        <?php
                                        $fb_profile      = get_user_meta($user->id, 'fb_profile');
                                        $gp_profile      = get_user_meta($user->id, 'gp_profile');
                                        $twitter_profile = get_user_meta($user->id, 'twitter_profile');
                                        $li_profile = get_user_meta($user->id, 'li_profile');
                                        ?>
                                        <div class="brand-bg">
                                            <?php if ($li_profile!=''){?>
                                            <a class="vk" href="https://<?php echo $li_profile; ?>" target="_blank" rel="nofollow"><i class="fa fa-vk circle-3"></i></a>
                                            <?php }?>
                                            <?php if ($fb_profile!=''){?>
                                            <a class="facebook" href="https://<?php echo $fb_profile; ?>" target="_blank" rel="nofollow"><i class="fa fa-facebook circle-3"></i></a>
                                            <?php }?>
                                            <?php if ($twitter_profile!=''){?>
                                            <a class="twitter" href="https://<?php echo $twitter_profile; ?>" target="_blank" rel="nofollow"><i class="fa fa-twitter circle-3"></i></a>
                                            <?php }?>
                                             <?php if ($gp_profile!=''){?>
                                            <a class="google-plus" href="https://<?php echo $gp_profile; ?>" target="_blank" rel="nofollow"><i class="fa fa-google-plus circle-3"></i></a>
                                            <?php }?>
                                       </div>
                                    </div>
