...
if (!function_exists('get_city_name_by_code')){
    function get_city_name_by_code($code){
        if($code == 0){
            return '';
        }
        ...
        $CI = get_instance();
        $CI->load->database();
        $query = $CI->db->get_where('city',array('city_code'=>$code));
        
        if ($query->num_rows() > 0){
            $row = $query->row();
            return lang_key($row->city_name);
        }
        else{
            return 'N/A';
        }
...
if ( ! function_exists('show_price'))
{
	function show_price ($price,$currency)
	{
		$CI = get_instance();
		$CI->load->database();
		$query = $CI->db->get_where('currency',array('code'=>$currency));
		
                if ($query->num_rows()>0){
                    $row = $query->row();
                    return $price." ".lang_key($row->symbol);
		}
		else{
                    return $price." N/A";
		}
		...
		{
			return $CI->session->userdata('system_currency').''.number_format($price, 2, $decimal_point, $thousand_separator);
		}
		...
		{
			return number_format($price, 2, $decimal_point, $thousand_separator).''.$CI->session->userdata('system_currency');
		}
		else
		{
			return number_format($price, 2, $decimal_point, $thousand_separator).' '.$CI->session->userdata('system_currency');
...
if ( ! function_exists('get_all_currencies'))
{
	function get_all_currencies($key=0)
	{
		$currencies= array(
			...
			"ILS"=> array("�?зраиль, Новый шекель", "20aa"),
			...
			"руб"=> array("Российская Федерация, Рубли", "20BD"),
...
if ( ! function_exists('get_status_title_by_value'))
{
	...
	{
		$types = array("DBC_DELETED","DBC_ACTIVE","DBC_CLOSED","DBC_PENDING","DBC_PAYMENT_PENDING","DBC_EXPIRED");
...
if ( ! function_exists('get_vip_status_title_by_value')) 
{
	function get_vip_status_title_by_value($key=0, $period = 0)
	{
                $types = array("no","yes");
		$front = '';
		$back  = '</span>';
...
if ( ! function_exists('get_vip_status_by_value'))
{
	function get_vip_status_by_value($key=0, $period = 0)
	{
            if (time() - $key < $period){
                return 1;
            }
...
if ( ! function_exists('get_expired_title')) {
	function get_expired_title($timestamp=0, $period = 0){
            $front = '';
            $result = null;
            $back  = '</span>';
            ...
            return $result != null ? $front.lang_key($result).$back : "N/A";
	}
}

if ( ! function_exists('get_expired_flag')) {
	function get_expired_flag($timestamp=0, $period = 0){
            $result = null;
...
if ( ! function_exists('get_all_cities_in_use'))
{
	function get_all_cities_in_use()
	{
		$CI = get_instance();
		$CI->load->database();
		$sql = 'select distinct(city) from '.$CI->db->dbprefix('posts').' where status=1';
		...
		$query = $CI->db->get_where('locations',array('status'=>1));
		return $query;
	}
}
...
{
	function add_user_meta ($user_id,$key,$value) // 5, coins, 100
	{
		$CI = get_instance();
		$CI->load->database();
		//$query = $CI->db->get_where('user',array('num'=>$user_id,'key'=>$key));
		//if($query->num_rows()>0)
		//{
                $CI->db->update('user', array($key=>$value), array('num'=>$user_id));
		//}
		//else
		//{
...
if ( ! function_exists('get_featured_photo_by_id'))
{
	function get_featured_photo_by_id($img = ''){
            if($img == '' || $img == "[]"){
                ...
                return 'http://'.$_SERVER["HTTP_HOST"].'/catalog/'.$parsed[0]; // ����� ����� ������� ���� � ������� �� ����� �����
                //return base_url('assets/admin/img/preview.jpg');
            }
	}
}
	...
	function get_post_data_by_lang($post,$column='title',$lang='')
	{
		...
		if($column=='title')
		{
			...
			if(isset($titles->{$lang}) &&  $titles->{$lang}!='')
				...
				return (isset($titles->{default_lang()}));
		}
		else
		{
			$descriptions = json_decode($post->description);
...
if ( ! function_exists('social_sharing_meta_tags_for_post'))
{
	...
	function social_sharing_meta_tags_for_post($post='')
	{
		if($post!='' && $post->num_rows()>0)
		{
            ...
            $featured_img = base_url('uploads/images/'.$post->featured_img);
            //$featured_img = get_featured_photo_by_id($post->featured_img);
...
if(!function_exists('social_sharing_meta_tags_for_blog'))
{
    ...
    function social_sharing_meta_tags_for_blog($blog_meta='')
    {
        if($blog_meta!='')
        {
            ...
            $image_path=(!empty($blog_meta->featured_img)? base_url().'uploads/images/'.$blog_meta->featured_img : base_url().'assets/admin/img/preview.jpg');
...
{
	function configPagination($url,$total_rows,$segment,$per_page=100, $order_by)
	{
		$CI = get_instance();
...
if ( ! function_exists('get_author_name_by_id'))
{
	function get_author_name_by_id($id='')
	{
		...
		$CI = get_instance();
		$CI->load->database();
		$query = $CI->db->get_where('user',array('num'=>$id));
		if($query->num_rows()>0)
		{
			$row = $query->row();
			//return lang_key($row->category_name);
                        return $row->name;
		}
		else
			return '';
...
if ( ! function_exists('get_category_title_by_id'))
{
	function get_category_title_by_id($id='')
	{
		...
		$CI = get_instance();
		$CI->load->database();
		$query = $CI->db->get_where('category',array('category_code'=>$id));
		if($query->num_rows()>0)
		{
			$row = $query->row();
			//return lang_key($row->category_name);
...
if ( ! function_exists('get_profile_photo_by_id'))
{
	function get_profile_photo_by_id($id='',$type=''){
            ...
            if($query->num_rows()>0){
                ...
                if($row->photo_100 == "")
                    return base_url().'uploads/profile_photos/nophoto-male.jpg';

                if ($row->vk_id != "" || $row->ok_id != ""){
                    return $row->photo_100;
                }
                
                if($type=='thumb')
                    return base_url().'users/'.$row->photo_100;
                else
                    return base_url().'users/'.$row->photo_100;
            }
            else{
...
if ( ! function_exists('get_all_classifieds_map_data')){
    function get_all_classifieds_map_data(){    	
        ...
        $CI->load->database();
        $query 	= $CI->db->get_where('catalog', array('status'=> 1),$limit,0);
        $data 	= array();
        $posts 	= array();
	$i=0;
...
{
	function get_formatted_address($address, $city, $state, $country,$hide_address='')
	{
		if($hide_address==1)
		return get_location_name_by_id($city).', '/*dymm removed .get_location_name_by_id($state).', '*/.get_location_name_by_id($country);
		else
		return $address.', '.get_location_name_by_id($city).', '/* dymm removed.get_location_name_by_id($state).', '*/.get_location_name_by_id($country);
	}
}
...
if ( ! function_exists('remove_feature_expiration'))
{
	function remove_feature_expiration ($today)
	{
		...
		$CI->db->update('dbc_posts',array('featured'=>0));
	}
}
...
if ( ! function_exists('remove_post_expiration'))
{
	function remove_post_expiration ($today)
	{
		...
		$CI->db->update('dbc_posts',array('status'=>4));
	}
}
...
if ( ! function_exists('get_post_sell_count'))
{
	function get_post_sell_count()
	{
		...
		$CI->db->where('status',1);
		//$query = $CI->db->get_where('catalog',array('purpose'=>'sell'));
...
if ( ! function_exists('translateable_date'))
{
	function translateable_date($time)
	{
		...
		return ' '.$day.' '.lang_key(strtolower($mon)).' '.$year;
	}
}
