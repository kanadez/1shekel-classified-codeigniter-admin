...
if ( ! function_exists('get_all_currencies'))
{
	function get_all_currencies($key=0)
	{
		$currencies= array(
			...
			"ILS"=> array("Израиль, Новый шекель", "20aa"),
...
if ( ! function_exists('get_formatted_address'))
{
	...
	{
		if($hide_address==1)
		return get_location_name_by_id($city).', '.get_location_name_by_id($state).', '.get_location_name_by_id($country);
		else
		return $address.', '.get_location_name_by_id($city).', '.get_location_name_by_id($state).', '.get_location_name_by_id($country);
	}
}
