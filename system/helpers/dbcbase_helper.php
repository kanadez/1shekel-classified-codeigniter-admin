		...
		if(is_installed('return')!='no')
		{
			...
			{	
				if(is_installed('return')=='yes')
				{
					$CI = get_instance();
					$CI->load->database();
					$query 		= $CI->db->get_where('dbc_options',array('key'=>'site_settings'));		
					if($query->num_rows()>0)
					{
						$row = $query->row();
...
if ( ! function_exists('get_user_title_by_id'))
{
	function get_user_title_by_id($id)
	{
		$CI = get_instance();
		$CI->load->database();
		$query = $CI->db->get_where('dbc_users',array('id'=>$id));
		if($query->num_rows()>0)
		{
			$row = $query->row();
			if($row->first_name!='')
...
if ( ! function_exists('get_per_page_value'))
{
	...
	{
		return get_settings('classified_settings','per_page',20);		
	}
}
...
{
	function configPagination($url,$total_rows,$segment,$per_page=10, $order_by)
	{
...
if ( ! function_exists('get_option'))
{
	function get_option($key='')
	{
		...
		if($defined==0)
		{
			$CI = get_instance();
			$CI->load->database();
			$query = $CI->db->get_where('dbc_options',array('key'=>$key,'status'=>1));		
			if($query->num_rows()>0)
				$option = $query->row();
			else
...
if ( ! function_exists('add_option'))
{
	function add_option($key='',$values='')
	{
		...
		{
			$query = $CI->db->update('dbc_options',$data,array('key'=>$key));		
		}
	}
}
...
if ( ! function_exists('truncate'))
{
	function truncate($s, $l, $e = '...', $isHTML = false){
		...
		return mb_substr($s, 0, $l = min(mb_strlen($s,'UTF-8'),  $l + $i),'UTF-8') . (count($tags = array_reverse($tags)) ? '</' . implode('></', $tags) . '>' : '') . (strlen($s) > $l ? $e : '');
	}
}
